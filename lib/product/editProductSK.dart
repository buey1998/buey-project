...
class DetailsProductSK extends StatefulWidget {
  // DetailsProductSK({Key key}) : super(key: key);

  final Skrice skricedata;
  final docs;

  DetailsProductSK({this.skricedata, this.docs, Skrice ptricedata});

  @override
  _DetailsProductSKState createState() => _DetailsProductSKState();
}

class _DetailsProductSKState extends State<DetailsProductSK> {
    ...
    if (textOnSave()) {
      try {
        ...
        return addDatoToFirebase
            .doc(widget.docs)
            .update({
              'name': widget.skricedata.name,
              'details': widget.skricedata.details,
              'setproduct': widget.skricedata.setproduct,
              'price': widget.skricedata.price,
            })
            .then((value) => {Navigator.pop(context)})
            .catchError((error) => print("Failed to add user: $error"));
      } catch (e) {
  ...
  Future<void> _deleteProduct(BuildContext context) async {
    var count = 0;
    if (widget.skricedata.image != null) {
      StorageReference storageReference = await FirebaseStorage.instance
          .getReferenceFromUrl(widget.skricedata.image);
            ...
            children: [
                ...
                children: [
                  Container(
                    ...
                    child: imageFile == null
                        ? Image.network('${widget.skricedata.image.toString()}')
              ...
              Padding(
                ...
                child: InkWell(
                  onTap: () async {
                    if (urlPicture == null) {
                      urlPicture = widget.skricedata.image.toString();
                    }
